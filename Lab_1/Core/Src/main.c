...
int main(void) {
	...
	// Set pins PC6 and PC7 to general purpose output mode
	GPIOC->MODER |= (1 << 14) | (1 << 12);
	GPIOC->MODER &= ~((1 << 15) | (1 << 13));
	// Set pins PC6 and PC7 to push pull output type
	GPIOC->OTYPER &= ~((1 << 15) | (1 << 14) | (1 << 13) | (1 << 12));
	// Set pins PC6 and PC7 to low speed
	GPIOC->OSPEEDR &= ~((1 << 15) | (1 << 14) | (1 << 13) | (1 << 12));
	// Set pins PC6 and PC7 to no pullup/down resistor
	...
	//Initialize pin 6 to off and 7 to on
	GPIOC->ODR |= (1 << 7);
	GPIOC->ODR &= ~(1 << 6);
